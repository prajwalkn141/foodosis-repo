...
# Import from the published PyPI package
from food_safety_lib import get_expiry_status, get_safety_score, calculate_shelf_life, generate_alert_message, get_batch_alerts, calculate_waste_cost
...
def add_item():
    ...
    if request.method == 'POST':
        ...
        print(f"Loaded AWS_ACCESS_KEY_ID: {fresh_access_key}")
        print(f"Loaded S3_BUCKET: {fresh_s3_bucket}")
        ...
        if file and file.filename:
            try:
                ...
                print(f"Attempting S3 upload for key: {key}")
        ...
        item_id = None
        try:
            print(f"Attempting to add item to RDS: Name={name}, Quantity={quantity}, Unit={unit}, Expiration={expiration_date_str}, S3_Key={s3_file_key}")
            item_id = rds_utils.add_item(name, quantity, unit, expiration_date_str, s3_file_key)
            print(f"Item added to RDS successfully. Item ID: {item_id}")
            ...
            if expiration_date_str:
                ...
                if days_remaining <= 7:
                    print(f"Item '{name}' (ID: {item_id}) expires in {days_remaining} days. Invoking Lambda for immediate check.")
                    ...
                    print(f"Item '{name}' (ID: {item_id}) expires in {days_remaining} days. No immediate check needed.")
